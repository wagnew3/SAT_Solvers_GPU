...
public final class SolverFactory extends ASolverFactory<ISolver> {
    ...
    public static Solver<DataStructureFactory> newMiniLearningHeapML() {
        return newMiniLearningHeapML(new MixedDataStructureDanielWL());
    }
    ...
    public static Solver<DataStructureFactory> newMiniLearningHeapExpSimpML() {
        Solver<DataStructureFactory> solver = newMiniLearningHeapML();
        solver.setSimplifier(solver.EXPENSIVE_SIMPLIFICATION);
        return solver;
    }
    ...
    public static Solver<DataStructureFactory> newMiniLearningHeapRsatExpSimpML(SATSolverSAT4J mlSAT, int numberVariables) {
        Solver<DataStructureFactory> solver = newMiniLearningHeapExpSimpML();
        //solver.setOrder(new VarOrderHeap(new RSATPhaseSelectionStrategy()));
        solver.setOrder(new MLVarOrderHeap(new RSATPhaseSelectionStrategy(), mlSAT, solver.constrs, numberVariables));
        return solver;
    }
    ...
    public static ICDCL<DataStructureFactory> newGlucose21ML(SATSolverSAT4J mlSAT, int numberVariables) {
        Solver<DataStructureFactory> solver = newMiniLearningHeapRsatExpSimpML(mlSAT, numberVariables);
        solver.setRestartStrategy(new Glucose21Restarts());
        solver.setLearnedConstraintsDeletionStrategy(solver.lbd_based);
        return solver;
    }
    ...
    public static Solver<DataStructureFactory> newMiniLearningHeapML(
            DataStructureFactory dsf) {
        return newMiniLearningML(dsf, new VarOrderHeap());
    }
    ...
    public static Solver<DataStructureFactory> newMiniLearningML(
            DataStructureFactory dsf, IOrder order) {
        // LimitedLearning<DataStructureFactory> learning = new
        // PercentLengthLearning<DataStructureFactory>(10);
        MiniSATLearning<DataStructureFactory> learning = new MiniSATLearning<DataStructureFactory>();
        Solver<DataStructureFactory> solver = new MLSolver<DataStructureFactory>(
                learning, dsf, order, new MiniSATRestarts());
        return solver;
    }
    ...
    public static MLSolver newDefaultML(SATSolverSAT4J mlSAT, int numberVariables) {
        return (MLSolver) newGlucose21ML(mlSAT, numberVariables); // newMiniLearningHeapRsatExpSimpBiere();
    }
