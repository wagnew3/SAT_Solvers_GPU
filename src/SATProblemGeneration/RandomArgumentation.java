...
public class RandomArgumentation extends SATProblemGenerator 
{
	...
	public SAT generateSAT() 
	{
		...
		double fracTrue=0.1;
		int numberTrue=(int)Math.round(numberVariables*fracTrue);
		List<Variable> cloneVarVector=new ArrayList<>();
		for(Variable variable: varVector)
		{
			cloneVarVector.add(variable);
		}
		Collections.shuffle(cloneVarVector);
		List<Variable> variables=new ArrayList<>();
		List<Integer> nots=new ArrayList<>();
		for(int reqTrue=0; reqTrue<numberTrue; reqTrue++)
		{
			variables.add(cloneVarVector.remove(0));
			nots.add(0);
		}
		Clause clause=new Clause(variables, nots);
		clauses.add(clause);
