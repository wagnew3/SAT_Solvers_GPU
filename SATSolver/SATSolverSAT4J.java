...
public class SATSolverSAT4J 
{
	...
	public void setFromSAT(SAT sat)
	{
		...
		{
			for(int varInd=0; varInd<sat.clauses.get(varInd).variables.size(); varInd++)
			{
				if(sat.clauses.get(clauseInd).nots.get(varInd)!=((OriginalWLClause)((Solver)solver).constrs.get(clauseInd)).get(varInd)%2)
				{
					int u=0;
				}
			}
